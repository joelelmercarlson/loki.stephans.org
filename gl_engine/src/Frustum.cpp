...
// load frustum from current matrix
...
{
	return SphereInFrustum(x, y, z, 0.0f);
}
...
// alternative is return d + radius to get distance
float Frustum::f_SphereInFrustum(float x, float y, float z, float radius)
{
	float d;
	for (int p = 0; p < 6; p++)
	{
