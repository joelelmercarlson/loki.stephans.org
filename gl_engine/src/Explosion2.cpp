...
Explosion2::Explosion2(const std::string& filename, int numParticles, const Vector& vec, float spread) : 
...
void Explosion2::InitializeParticle(int index)
{
	// position and size
	...
	m_particleList[index].m_velocity[0] = PARTICLE_VELOCITY2[0] + FRAND * VELOCITY_VARIATION2[0];
	m_particleList[index].m_velocity[1] = PARTICLE_VELOCITY2[1] + FRAND * VELOCITY_VARIATION2[1];
	m_particleList[index].m_velocity[2] = PARTICLE_VELOCITY2[2] + FRAND * VELOCITY_VARIATION2[2];
	m_particleList[index].m_acceleration = PARTICLE_ACCELERATION2;

	// colors in yellow and red
	m_particleList[index].m_color[0] = 0.5f*FRAND;
	m_particleList[index].m_color[1] = 0.3f*FRAND;
	m_particleList[index].m_color[2] = 1.0f;
	m_particleList[index].m_color[3] = 1.0f;

	// how much energy?
	m_particleList[index].m_energy = 0.9f + FRAND/2.0f;
...
void Explosion2::Update(float elapsedTime)
{
